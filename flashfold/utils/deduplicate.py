...
def get_min_num_of_diverse_hits(fasta_path: str, outfile: str, min_sequences: int = 1000, threads: int = 8) -> None:
    """
    ...
    for i in range(1, 10):
        identity = f"{i/10:.2f}"
        out_dir = os.path.join(root_out_dir, f"cd_hit_{identity}")
        ...
        if len(seq_records) >= min_sequences or i == 9:
            with open(outfile, 'w') as out_file:
